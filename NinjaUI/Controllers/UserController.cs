...
namespace NinjaUI.Controllers
{
    public class UserController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> PushUserToSnapchat()
        {
            List<UserDto> respons = new List<UserDto>();
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> PushUserToSnapchat(UserDto userDto)
        {
            var client = httpClientFactory.CreateClient();
            var httpRequestMessage = new HttpRequestMessage()
            {
                Method = HttpMethod.Post,
                RequestUri = new Uri("https://localhost:7054/api/User/Push_User"),
                Content = new StringContent(JsonSerializer.Serialize(userDto), Encoding.UTF8, "application/json"),
            };
            var httpResponseMessage = await client.SendAsync(httpRequestMessage);
            httpResponseMessage.EnsureSuccessStatusCode();
